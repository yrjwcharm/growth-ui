...
const Index = (props) => {
  ...
  useEffect(() => {
    ...
    chartRef.current
      ...
      .style('type', {
        lineWidth: function (val: string | string[]) {
          if (val.includes('本基金') || val.includes('买卖')) {
            return 1
          } else {
            return 0.5
          }
      ...
      .shape('type', function (type: string) {
        if (type === '成本价') {
          return 'dash'
        }
      })
    ...
    if (onLastHalfAreaChange) {
      ...
      chartRef.current.on('beforerender', () => {
        const end = chartRef.current.getPosition(last)
